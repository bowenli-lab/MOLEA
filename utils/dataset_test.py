...
# Function to safely convert values to float while printing non-numeric values
# def safe_float_convert(x):
#     try:
#         return float(x)  # Convert to float
#     except ValueError:
#         print(f"Non-numeric value found: {x}")  # Print problematic value
...
class MolTestDatasetWithAdditionalFeatures(MolTestDataset):
    def __init__(self, data_path, target, task, feature_cols):
        ...
        # print("247: ", self.feature_cols)
        
        # # Convert `self.feature_cols` (assumed to be a list of lists) to a NumPy array
        # self.feature_cols = np.array([[safe_float_convert(x) for x in row] for row in self.feature_cols], dtype=np.float32)
