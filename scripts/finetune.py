...
class FineTune(object):
    def __init__(self, dataset, config):
        ...
        log_dir = os.path.join("finetune_Hepatocyte", dir_name)
    ...
    def _load_pre_trained_weights(self, model):
        try:
            ...
            state_dict = torch.load(
                os.path.join(checkpoints_folder, "model_14.pth"), map_location=self.device
            )
...
def main(config):
    ...
    model_finetune = FineTune(dataset, config)
    model_finetune.train()

    if config["dataset"]["task"] == "regression":
        model_finetune.res = model_finetune.rmse
...
if __name__ == "__main__":
    ...
    if config["task_name"] == "lnp_muye_experiments":
        ...
        config["dataset"][
            "data_path"
        ] = "/home/sdl_r/LNP_4CR_muye/finetune_dataset/finetune_data.csv"
        target_list = ["mTP_(Log2)_Hepatocyte"]
