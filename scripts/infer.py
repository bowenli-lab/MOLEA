...
from utils.dataset_test import MolTestDatasetWrapper, MolTestDataset
...
class Inference(object):
    def __init__(self, dataset, config):
        ...
        self.log_dir = os.path.join("finetune_Hepatocyte", config["model_to_evaluate"]) # TODO change this
    ...
    def inference(self):
        ...
        self.config["model"].pop("norm", None)
        from molea_finetune import MOLEA
        model = MOLEA(self.config["dataset"]["task"], **self.config["model"]).to(
            self.device
        )
        ...
        if self.config["dataset"]["task"] == "regression":
            ...
            if self.config["task_name"] in ["qm7", "qm8", "qm9"]:
                ...
                # print("202: labels: ", labels)
                # print("202: predictions: ", predictions)
        ...
        return predictions, embeddings, labels, all_smiles
    ...
    def _load_pre_trained_weights(self, model):
        try:
            checkpoints_folder = os.path.join(
                "./finetune_Hepatocyte", self.config["model_to_evaluate"], "checkpoints" # TODO change this directory
            )
...
if __name__ == "__main__":
    ...
    config_file = os.path.join(
        "./finetune_Hepatocyte", args.checkpoint, "checkpoints", "config_finetune.yaml" # TODO change this directory
    )
    config = yaml.load(open(config_file, "r"), Loader=yaml.FullLoader)
    config["model_to_evaluate"] = args.checkpoint

    if config["task_name"] == "lnp_muye_experiments":
        config["dataset"]["task"] = "regression"
        config["dataset"][
            "data_path"
        ] = "<Path to candidates library>"
        target_list = ["desc_ABC"]
    ...
    for target in target_list:
        ...
        pred, embs, labels, all_smiles = infer_agent.inference()
        results_list.append(
            {
    ...
    highlight_smiles = ['CCCCCCCCCCCCCCCCCC(=O)N(CCN1CCNCC1)C(CCCCCCCCCCCCCCCCC)C(=O)NC1CCCCC1', 
                        'CCCCCCCCCCCCCCCCCC(=O)N(CCN1CCNCC1)C(CCCCCCCCCCCCCCCC)C(=O)NC1CCCCC1',
                        'CCCCCCCCCCCCCCCCCC(=O)N(CCN1CCNCC1)C(CCCCCCCCCCCCCCC)C(=O)NC1CCCCC1',
                        'CCCCCCCCCCCCCCCCCC(=O)N(CCN1CCNCC1)C(CCCCCOC(=O)C(CCCCCC)CCCCCCCC)C(=O)NCC(=O)OCC',
                        'CCCCCCCCCCCCCCCC(=O)N(CCN1CCNCC1)C(CCCCCOC(=O)C(CCCCCC)CCCCCCCC)C(=O)NCC(=O)OCC',
                        'CCCCCCCCCCCCCCCCCC(=O)N(CCN1CCNCC1)C(CCCCCOC(=O)C(CCCC)CCCCCC)C(=O)NCC(=O)OCC',
                        'CCCCCCCC/C=C\CCCCCCCC(=O)N(CCN1CCNCC1)C(CCCCCOC(=O)C(CCCCCC)CCCCCCCC)C(=O)NCC(=O)OCC',
                        'CCCCCCCC/C=C\CCCCCCCC(=O)N(CCN1CCNCC1)C(CCCCCOC(=O)C(CCCCCC)CCCCCCCC)C(=O)NCC(=O)OC',
                        'CCCCCCCC/C=C\CCCCCCCC(=O)N(CCN1CCNCC1)C(CCCCCOC(=O)C(CCCCCC)CCCCCCCC)C(=O)NC1CCCCC1']  # A list of SMILES to highlight

    for highlight_smile in highlight_smiles:
        if highlight_smile in smiles_data_:

            index = smiles_data_.index(highlight_smile)
            x = infer_agent.umap_emb[index, 0]
            y = infer_agent.umap_emb[index, 1]
            if highlight_smile == "CCCCCCCC/C=C\CCCCCCCC(=O)N(CCN1CCNCC1)C(CCCCCOC(=O)C(CCCCCC)CCCCCCCC)C(=O)NC1CCCCC1":
                ax.scatter(
                    x,
                    y,
                    s=10,  # larger size
                    c='red',  # transparent fill
                    edgecolors=None,  # black border
                    # linewidths=2,
                    marker="o",
                    label=f"Highlight: {highlight_smile}"
                )
            else:
                ax.scatter(
                    x,
            ...
            print(f"SMILES {highlight_smile} not found in dataset.")
    ...
    fig.savefig(
        os.path.join(
            infer_agent.log_dir,
            f"{feature_name}_{config['model_to_evaluate']}_{config['task_name']}_umap.svg",
        )
    )
